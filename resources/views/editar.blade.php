...
@section('contenido')
    <div class="max-w-xl mx-auto bg-white shadow-lg rounded-lg p-8">
        <form class="space-y-4" method="POST" action="{{ route('articles.update', $article) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <!-- NOMBRE -->
            <div>
                ...
                <input type="text" id="nombre" name="nombre" value="{{ @old('nombre', $article->nombre) }}"
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="4"
                    class="w-full border border-gray-300 rounded-md p-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Descripción del artículo">{{ @old('descripcion', $article->descripcion) }}</textarea>
                ...
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2 text-gray-700">
                        <input type="radio" name="disponible" value="Si" class="text-blue-600 focus:ring-blue-500"
                            @checked(@old('disponible', $article->disponible) == 'Si')>
                        <span>Si</span>
                    </label>

                    <label class="flex items-center gap-2 text-gray-700">
                        <input type="radio" name="disponible" value="No" class="text-blue-600 focus:ring-blue-500"
                            @checked(@old('disponible', $article->disponible) == 'No')>
                        <span>No</span>
                    </label>
                </div>
            ...
            <div class="p-1 rounded bg-green-100 relative w-full h-82 border-2 border-black">
                ...
                <img src="{{ Storage::url($article->imagen) }}" class="w-full h-full bg-center bg-cover bg-no-repeat" id="preview" />
            </div>
